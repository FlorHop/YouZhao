...
首次启动时需要在登录页初始化管理员账号。平台不再内置演示账号。

MCP Token 需要在管理员初始化后进入系统设置创建。后端本地开发可按需要手动创建 Token 并用于 MCP Server：
...
- `1.2.2`：移除登录页演示账号提示，首次部署改为先初始化管理员帐号，初始化后再进入正常登录。
- `1.2.1`：修复点击蓝图会新开页签的问题，改为在当前页签打开预览并返回蓝图模块。
